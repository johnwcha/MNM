<template>
  <v-app dark>
    <v-navigation-drawer v-model="drawer" fixed app
      :mini-variant="miniVariant" :clipped="clipped">
      <!-- <v-list>
        <v-list-item class="px-2">
          <v-list-item-avatar>
            <v-img aspect-ratio="1" :src="require(`../static/assets/`+_animals[0])"></v-img>
          </v-list-item-avatar>
        </v-list-item>
     ...
     <v-divider></v-divider>
      <v-list>
        <v-list-item v-for="(item, i) in items" router exact
          :key="i" :to="item.to">
          <v-list-item-action>
    ...
    <v-app-bar :clipped-left="clipped" flat app hide-on-scroll>
...
export default {
  data () {
    return {
      ...
      right: true,
      rightDrawer: false,
      title: 'Chinese Grammar Exemplified @ YouTube'
    }
  },
  computed: {
    items () {
    ...
    _animals() {
      return this.$store.state._animals
    }
  },
  mounted() {
    ...
    // var _animals = []
    // temp.forEach(item => {
    //     var temp = {icon: item, player: ''}
    //     _animals.push(temp)
    // })
    // console.log(_animals.length, _animals)
    //console.log('default vue loaded')
  }
}
</script>
